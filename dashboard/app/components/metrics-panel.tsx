...
export function MetricsPanel({ summary }: { summary: DashboardSummary | null }) {
  ...
  return (
    <div className="metric-grid">
      ...
      <article className="metric slate"><p>Idle</p><strong>{formatDuration(summary?.idleSeconds || 0)}</strong><small>{topLeftRunning
        ? `no input · ${topLeftRunning.name} left running ${formatDuration(topLeftRunning.seconds)}`
        : "screen open, no activity"}</small></article>
